...
messageApi.redGet("getMessages", "/", {
  ...
  const {
    messages,
    total
  } = await messageDao.getMessages(ctx, start, count);
  if (total < 1) {
    throw new NotFound({
      msg: "未找到任何消息"
    });
...
messageApi.redPut("readMessage", "/one/:id", {
  auth: "标记一条消息为已读",
  module: "推送",
  mount: false
}, loginRequired, async (ctx) => {
...
messageApi.redPut("readMessages", "/some", {
  auth: "标记多条消息为已读",
  module: "推送",
  mount: false
}, loginRequired, async (ctx) => {
...
messageApi.redDelete("deleteMessage", "/:id", {
  auth: "删除一条消息",
  module: "推送",
  mount: false
}, loginRequired, async (ctx) => {
