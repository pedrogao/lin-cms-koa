...
import { broker, notify, MESSAGE_EVENTS } from "../middleware";
...
sseApi.redGet(
  "stream",
  "/",
  { auth: "消息推送", module: "推送", mount: true },
  groupRequired,
  async ctx => {
...
sseApi.redGet(
  "getEvents",
  "/events",
  { auth: "获得events", module: "推送", mount: true },
  groupRequired,
  notify("测试一条消息", "test"),
  async ctx => {
    const currentUser: UserInterface = ctx.currentUser;
    if (currentUser.isSuper) {
      ctx.json({ events: Array.from(MESSAGE_EVENTS.values()) });
      return;
    }
...
sseApi.redPut(
  "putEvents",
  "/events",
  { auth: "更新events", module: "推送", mount: false },
  adminRequired,
  async ctx => {
    const form = new EventsForm(ctx).validate();
    await eventDao.updateEvents(form);
    ctx.json(new Success({ msg: "更新成功" }));
  }
);
